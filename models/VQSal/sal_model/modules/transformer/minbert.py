...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, attention_mask, layer_past=None):
        ...
        if attention_mask is not None:
            att = att.masked_fill(attention_mask[:,:,:T,:T] == 0, float('-inf'))
...
class BERT(nn.Module):
    ...
    def forward(self, idx, embeddings=None, targets=None, attention_mask=None):
        # forward the GPT model
        token_embeddings = self.tok_emb(idx) # each index maps to a (learnable) vector. |<<(e.g. token_embeddings.shape [2,511,1024])>>|
        ...
        x = self.ln_f(x)
        logits = self.head(x)   #|<<(e.g. token_embeddings.shape [2,511,1024])>>|
